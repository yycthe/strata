...
export async function importOwnersFromCsv(csvData: string): Promise<{ success: boolean; message: string; }> {
    try {
        const parsed = Papa.parse(csvData, { header: true, skipEmptyLines: true });
        
        if (parsed.errors.length > 0) {
            return { success: false, message: `CSV Parsing Error: ${parsed.errors[0].message}` };
        }
        
        const rows = z.array(CsvRowSchema).parse(parsed.data);

        const upsertPropertyStmt = db.prepare(`
            INSERT INTO properties (name, address_1, city, state, postal_code, plan_code, unit_number)
            VALUES (@name, @address_1, @city, @state, @postal_code, @plan_code, @unit_number)
            ON CONFLICT(address_1) DO UPDATE SET name=excluded.name
            RETURNING id
        `);

        const getOwnerStmt = db.prepare('SELECT id, properties FROM owners WHERE email = ?');
        const insertOwnerStmt = db.prepare('INSERT INTO owners (name, email, properties) VALUES (?, ?, ?)');
        const updateOwnerStmt = db.prepare('UPDATE owners SET properties = ? WHERE id = ?');
        ...
        db.transaction(() => {
            for (const row of rows) {
                ...
                const property = upsertPropertyStmt.get({
                ...
                const owners = parseOwnerString(row['Rental owners']);
                for (const owner of owners) {
                    let existingOwner = getOwnerStmt.get(owner.email) as {id: number, properties: string} | undefined;
                    
                    if (existingOwner) {
                        const properties = JSON.parse(existingOwner.properties || '[]') as string[];
                        if (!properties.includes(propertyIdentifier)) {
                            properties.push(propertyIdentifier);
                            updateOwnerStmt.run(JSON.stringify(properties), existingOwner.id);
                        }
                    } else {
                        insertOwnerStmt.run(owner.name, owner.email, JSON.stringify([propertyIdentifier]));
                    }
                }
            }
        })();
        ...
        revalidatePath('/');
        return { success: true, message: `Successfully imported ${rows.length} rows.` };
